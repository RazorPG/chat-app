    ...
    <div class="relative z-10 w-full max-w-md animate-fade-in-up">
      ...
      >
        ...
        <div class="flex items-center justify-center gap-2 mb-8">
          <div
            class="w-2 h-2 transition-all duration-300 rounded-full bg-emerald-500"
          />
          <div
            :class="[
              'h-2 rounded-full transition-all duration-300',
              stage === 'otp' ? 'bg-emerald-500 w-8' : 'bg-emerald-500/30 w-2',
            ]"
          />
          <div
            :class="[
              'h-2 rounded-full transition-all duration-300',
              stage === 'set' ? 'bg-emerald-500 w-8' : 'bg-emerald-500/30 w-2',
            ]"
          />
        </div>

        <!-- Stage 1: OTP Verification -->
        <div v-if="stage === 'otp'" class="animate-slide-in" key="otp-stage">
          <!-- Logo and title -->
          ...
          <form @submit.prevent="handleVerify" class="space-y-5">
            <div class="space-y-2">
              <label class="text-sm font-medium text-emerald-400"
                >OTP Code</label
              >
              <input
                v-model="code"
        ...
        <div v-else class="animate-slide-in" key="password-stage">
          <!-- Logo and title -->
          <div class="flex flex-col items-center mb-8">
            <div
              class="relative p-4 mb-6 shadow-lg bg-gradient-to-br from-emerald-500 to-green-600 rounded-2xl shadow-emerald-500/50 animate-scale-in"
          ...
          <form @submit.prevent="handleSetPassword" class="space-y-5">
            ...
            >
              <template v-if="newPassword === confirmPassword">
                ...
                <span class="text-emerald-400">Passwords match</span>
              </template>
              <template v-else>
            ...
            <button
              ...
              class="w-full bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 text-white border-0 h-12 sm:h-13 rounded-xl shadow-lg shadow-emerald-500/50 transition-all hover:shadow-emerald-500/70 hover:scale-[1.02] active:scale-[0.98] font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {{ setting ? 'Updating...' : 'Reset Password' }}
            </button>
          </form>
          ...
          <div
            v-if="success"
            class="p-3 mt-4 border bg-emerald-500/10 border-emerald-500/30 rounded-xl"
          >
            <p class="text-sm text-center text-emerald-400">{{ success }}</p>
          </div>
        </div>
      </div>
